...
1. Download and unzip the `OpenCV` sources.
    ...
    XCS~$ unzip 4.3.0.zip 
    XCS~$ rm 4.3.0.zip
    XCS~$ wget https://github.com/opencv/opencv_contrib/archive/4.3.0.zip && unzip 4.3.0.zip && rm 4.3.0.zip 
    XCS~$ unzip 4.3.0.zip 
    XCS~$ rm 4.3.0.zip 
    ```
...
1. OpenCV comes with it's own toolchain for cross-compilation. Luckily CMake makes it possible to merge these files. See (rpi-generic-toolchain-opencv-4.3.0.cmake)[rpi-generic-toolchain-opencv-4.3.0.cmake] :
   ...
   > If you use a different OpenCV version, please make sure you edit this file accordingly.
...
1. Test the bindings:
    ...
    - python2:
    ```
    PYTHONPATH=/usr/local/lib/python2.7/site-packages python -c 'import cv2; print(cv2.__version__)'
    ```
    - python3:
    ```
    PYTHONPATH=/usr/local/lib/python3.7/site-packages python3 -c 'import cv2; print(cv2.__version__)'
    ```
...
1. A more permanent way to set `PYTHONPATH` is to use `.bashrc`
    ...
    > Note that you both python3 and python2 use the same `PYTHONPATH`, hence case should be taken when setting this in `.bashrc`: the different versions are not interchangeable!
...
> Code for this test is taken from http://docs.opencv.org/4.3.0/db/deb/tutorial_display_image.html

...
By default, the hostname of a rpi is `raspberrypi`, hence the rpi can be accessed via the dns `raspberrypi.local`. As multiple rpi's might be active in the environment, connection issues may occure. The following steps show how to change the hostname properly.
